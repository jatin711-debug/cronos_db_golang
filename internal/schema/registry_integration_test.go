...
func TestSchemaRegistryUnitValidation(t *testing.T) {
	...
	_, err = reg.Register(topic, TypeJSON, `{"type":"object","properties":{"user":{"type":"string"},"email":{"type":"string"}}}`)
	if err != nil {
...
func TestSchemaRegistryAvroValidation(t *testing.T) {
	...
	_, err = reg.Register(topic, TypeAvro, avroSchema)
	if err != nil {
		t.Fatalf("register avro schema: %v", err)
	}

	validPayload, err := encodeAvroForTest(avroSchema, map[string]interface{}{"name": "alice", "age": 30})
	if err != nil {
	...
	invalidPayload, err := encodeAvroForTest(avroSchema, map[string]interface{}{"name": "bob", "age": "thirty"})
	if err != nil {
...
func TestSchemaRegistryProtobufValidation(t *testing.T) {
	...
	_, err = reg.Register(topic, TypeProtobuf, "google.protobuf.Timestamp")
	if err != nil {
		t.Fatalf("register protobuf schema: %v", err)
	}

	validPayload, err := marshalTimestampForTest(time.Now())
	if err != nil {
...
// encodeAvroForTest encodes a Go map to Avro binary format.
func encodeAvroForTest(schemaJSON string, record map[string]interface{}) ([]byte, error) {
...
// marshalTimestampForTest encodes a time.Time to protobuf Timestamp binary.
func marshalTimestampForTest(ts time.Time) ([]byte, error) {
